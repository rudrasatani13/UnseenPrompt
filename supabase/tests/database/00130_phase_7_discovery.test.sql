...
select * from public.claim_generation_run_v3('project',(select project_id from tmp_project),9,'phase7-delta-key',repeat('1',64),'project_delta','unseenprompt.model-gateway-request.v3','unseenprompt.model-output.project_delta.v1');
select lives_ok($$select * from public.complete_generation_run_v3((select run_id from tmp_delta_claim),'succeeded','openai','model-phase7',10,4,5,0,1,'passed',null,'{"summary":"English discovery proposal","requirementProposals":[{"action":"add","reference":"","statement":"The notebook supports field researchers working offline.","rationale":"The confirmed discovery context identifies the audience and constraint."}],"decisionProposals":[],"milestoneProposals":[],"unresolvedConflicts":[]}',null)$$,'project delta remains on the existing validated path');
...
select * from public.claim_generation_run_v3('project',(select project_id from tmp_project),10,'phase7-delta-bad-key',repeat('2',64),'project_delta','unseenprompt.model-gateway-request.v3','unseenprompt.model-output.project_delta.v1');
select lives_ok($$select * from public.complete_generation_run_v3((select run_id from tmp_delta_bad),'succeeded','openai','model-phase7',10,4,5,0,1,'passed',null,'{"summary":"Boundary test","requirementProposals":[{"action":"add","reference":"","statement":"A requirement.","rationale":"Evidence."}],"decisionProposals":[{"action":"add","reference":"","statement":"A decision must not cross the boundary.","rationale":"Boundary test."}],"milestoneProposals":[],"unresolvedConflicts":[]}',null)$$,'boundary-test delta is valid Phase 6 output');
...
select * from public.claim_generation_run_v3('project',(select project_id from tmp_project),10,'phase7-delta-unapplied-key',repeat('4',64),'project_delta','unseenprompt.model-gateway-request.v3','unseenprompt.model-output.project_delta.v1');
select lives_ok($$select * from public.complete_generation_run_v3((select run_id from tmp_delta_unapplied),'succeeded','openai','model-phase7',10,4,5,0,1,'passed',null,'{"summary":"English discovery proposal","requirementProposals":[{"action":"add","reference":"","statement":"The notebook supports field researchers working offline.","rationale":"The confirmed discovery context identifies the audience and constraint."}],"decisionProposals":[],"milestoneProposals":[],"unresolvedConflicts":[]}',null)$$,'unapplied delta completion is valid');
