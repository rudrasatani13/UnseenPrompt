...
import { profilePatchSchema } from "@/domain/account/onboarding";
...
async function rejectMethod(): Promise<NextResponse> {
  ...
  return authErrorResponse("validation_failed", 405, { Allow: "PATCH" });
}
...
export async function PATCH(request: Request): Promise<NextResponse> {
  ...
  try {
    const profile = await createSupabaseAccountRepository(context.supabase).updateProfile(
      context.user.id,
      parsed.data,
    );
