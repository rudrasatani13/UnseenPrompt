...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const result = await service.remove(parsed.data.token);

    if (result === "removed" || result === "already_removed") {
      return NextResponse.json({ kind: "removed" }, { status: 200, headers: NO_STORE_HEADERS });
    }
