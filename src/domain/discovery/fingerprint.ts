...
/**
 * The question fingerprint is intentionally provider- and runtime-neutral:
 * NFC normalization, CRLF/CR to LF, trim, collapse Unicode whitespace to one ASCII space,
 * lowercase, UTF-8 encoding, then SHA-256. Database adapters must hash this canonical text.
 */
export const QUESTION_FINGERPRINT_CANONICALIZATION_V1 =
  "nfc|newlines_lf|trim|unicode_whitespace_to_ascii_space|lowercase|utf8|sha256" as const;

export function isWellFormedUnicodeStringV1(value: string): boolean {
...
export function normalizeQuestionFingerprintInputV1(value: string): string {
  if (typeof value !== "string" || !isWellFormedUnicodeStringV1(value)) {
